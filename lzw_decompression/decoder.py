...
class DecoderBytes():
    ...
    def init_table(self):
        ...
        # Clear Code
        self.code_table.append(None)
        # EOI Code
        self.code_table.append(None)

    def update_binary(self):
        self.binary = self.binary[self.nbits:]

    def add_table(self, value):
        """
        GIF最大4096字典
        """
        if len(self.code_table) < 4096:
            self.code_table.append(value)
    ...
    def update_nbits(self):
        ...
        """
        if self.nbits < 12:
            if len(self.code_table) == (1 << self.nbits):
                self.nbits += 1

    def decompression_lzw(self, code):
        """
        LZW核心
        """
        # 第一个code
        if self.prevcode is None:
            current = self.code_table[code]
            self.index_stream.extend(current)
            self.prevcode = code
            return
        table_size = len(self.code_table)
        if code < table_size:
            # 正常情况
            current = self.code_table[code]
            self.index_stream.extend(current)
            prev = self.code_table[self.prevcode]
            new_code = (
                prev +
                [current[0]]
            )
            self.add_table(new_code)
        elif code == table_size:
            """
            KwKwK特殊情况
            """
            prev = self.code_table[self.prevcode]
            ...
            self.index_stream.extend(new_code)
            self.add_table(new_code)
        else:
            raise ValueError(
                f"Invalid LZW code {code}, table size {table_size}"
            )
        self.prevcode = code
    ...
    def start(self):
        ...
        while True:
            # 数据不足
            if len(self.binary) < self.nbits:
                break
            code_bin = self.binary[:self.nbits]
            self.update_binary()
            code = self.parse_code(code_bin)
            print(
                "nbits=",
                self.nbits,
                "code=",
